class AddExpressionIndexesForCaliberAndClass < ActiveRecord::Migration[8.1]
  ...
  def change
    add_index :items, "(data->>'caliber')", name: "index_items_on_data_caliber", algorithm: :concurrently
    add_index :items, "(data->>'class')", name: "index_items_on_data_class", algorithm: :concurrently
  end
end
