...
module Admin
  class CraftUnlocksControllerTest < ActionDispatch::IntegrationTest
    ...
    test "get index with auth" do
      get_auth admin_craft_unlocks_path
      assert_response :success
      assert_select "h1", /Craft Unlocks/i
    end

    test "index has new button" do
      get_auth admin_craft_unlocks_path
      assert_response :success
      assert_match /New Craft Unlock/i, response.body
    end

    test "get show with auth" do
      get_auth admin_craft_unlock_path(@resource)
      assert_response :success
    end
    ...
    test "get new with auth" do
      get_auth new_admin_craft_unlock_path
      assert_response :success
      assert_match /New Craft Unlock/i, response.body
    end
    ...
    test "get edit with auth" do
      get_auth edit_admin_craft_unlock_path(@resource)
      assert_response :success
      assert_match /Edit Craft Unlock/i, response.body
    end

    test "update with valid params" do
      patch_auth admin_craft_unlock_path(@resource), params: { craft_unlock: { item_name: "Updated Craft" } }
      assert_redirected_to admin_craft_unlock_path(@resource)
    end
    ...
    test "index requires authentication" do
      get admin_craft_unlocks_path
      assert_response :unauthorized
    end

    test "show requires authentication" do
      get admin_craft_unlock_path(@resource)
      assert_response :unauthorized
    end
    ...
    test "new form renders item_id as a select with options" do
      get_auth new_admin_craft_unlock_path
      assert_response :success
      assert_match /<select[^>]*name="craft_unlock\[item_id\]"/, response.body
