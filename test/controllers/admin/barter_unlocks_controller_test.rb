...
module Admin
  class BarterUnlocksControllerTest < ActionDispatch::IntegrationTest
    ...
    test "get index with auth" do
      get_auth admin_barter_unlocks_path
      assert_response :success
      assert_select "h1", /Barter Unlocks/i
    end

    test "index has new button" do
      get_auth admin_barter_unlocks_path
      assert_response :success
      assert_match /New Barter Unlock/i, response.body
    end

    test "get show with auth" do
      get_auth admin_barter_unlock_path(@resource)
      assert_response :success
    end
    ...
    test "get new with auth" do
      get_auth new_admin_barter_unlock_path
      assert_response :success
      assert_match /New Barter Unlock/i, response.body
    end
    ...
    test "get edit with auth" do
      get_auth edit_admin_barter_unlock_path(@resource)
      assert_response :success
      assert_match /Edit Barter Unlock/i, response.body
    end

    test "update with valid params" do
      patch_auth admin_barter_unlock_path(@resource), params: { barter_unlock: { item_name: "Updated Item" } }
      assert_redirected_to admin_barter_unlock_path(@resource)
    end
    ...
    test "index requires authentication" do
      get admin_barter_unlocks_path
      assert_response :unauthorized
    end

    test "show requires authentication" do
      get admin_barter_unlock_path(@resource)
      assert_response :unauthorized
    end
    ...
    test "new form renders item_id as a select with options" do
      get_auth new_admin_barter_unlock_path
      assert_response :success
      assert_match /<select[^>]*name="barter_unlock\[item_id\]"/, response.body
