...
class TradersControllerTest < ActionDispatch::IntegrationTest
  ...
  test "show answers a fresh conditional request with 304" do
    trader = Trader.create!(bsg_id: "cond-#{SecureRandom.hex(4)}", slug: "cond-#{SecureRandom.hex(4)}",
                            name: "Conditional Trader", currency: "RUB")

    get trader_url(trader.slug)
    assert_response :success

    get trader_url(trader.slug), headers: { "If-None-Match" => response.headers["ETag"] }
